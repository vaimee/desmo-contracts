...
describe("DESMO Contracts tests", function() {
  ...
  describe("DESMO-LD Contract tests", function () {
    let hub: DesmoLDContract;
  
    beforeEach(async function () {
      const DESMCONTRACT = await ethers.getContractFactory("DesmoLDContract");
      hub = await DESMCONTRACT.deploy();
      await hub.deployed();
    });
    describe("DESMO-LD HUB - Transactions tests", function () {

      it("Should query receive result", async function () {
        const [addr1] = await ethers.getSigners();
        await hub.connect(addr1).receiveResult("0x05416460deb76d57af601be17e777b93592d8d4d4a4096c57876a91c84f4a712", "0x10220213ad2112e9");   
      });
